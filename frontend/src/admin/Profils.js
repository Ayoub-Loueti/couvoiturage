...
const Profils = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await axios.get('http://localhost:3006/autorise', {
          headers: {
  ...
  const handleBlockUser = async (userId) => {
    try {
      await axios.put(`http://localhost:3006/block/${userId}`, {}, {
        headers: {
          Authorization: `Bearer ${JSON.parse(token).token}`,
        },
      });
  ...
  return (
    <>
      ...
      <div className="container text-center mt-3">
        <h2>Profiles des utilisateurs</h2>
      </div>
      <div className="container text-center mt-3">
          ...
          {filteredUsers.map(
            (user) => (
              <tr key={user.id_utilisateur}>
                ...
                <td>{user.type}</td>
                <td>
                  <MDBBtn color="danger" onClick={() => handleBlockUser(user.id_utilisateur)}>
                    Bloquer
                  </MDBBtn>
                </td>
              </tr>
